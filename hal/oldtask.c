...
void task_management(void)
{
    static int tasknb=0, tasknb2, taskptr[2];
	...
	{
	    if (tssmap[tasknb].state == 1)
	    {
		tasklist[tasknb2] = tasknb;
		tasknb2++;
	    }
	}
	tasklist[tasknb2] = 0;
...
//putcar_drv (100);
//print ("Task run\n");
	for (tasknb2 = 0; tasklist[tasknb2] != 0; tasknb2++)
	{

        taskptr[1] = tssmap[tasklist[tasknb2]].gdt_desc * 8;

//putcar_drv (tasknb2 + 48);
//putcar_drv ((taskptr[1] / 8) + 48);
//while (1);
/*if (tasknb==5){tasknb=6; taskptr[1]=0x30;print ("6");goto go;}
if (tasknb==6){tasknb=5; taskptr[1]=0x28;print ("5");goto go;}
go:*/
...
char tss_find ()
{
    ...
    for (tasknb = 0; tasknb <= TSS_NB; tasknb++)//TSS 1&2 unreashable I don't know why
    {
